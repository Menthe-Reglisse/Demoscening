...
#define AMBIENT 0.8
#define INTENS (max(1.-floor(mod(iTime/2./PI,3.)),0.)==1.? 3.: max(1.-floor(mod(iTime/2./PI-1.,3.)),0.)==1.? 4.:2.)
#define EDGE_THICKNESS (max(1.-floor(mod(iTime/2./PI,3.)),0.)==1.? 0.015: max(1.-floor(mod(iTime/2./PI-1.,3.)),0.)==1.? 0.1:0.)
#define SHADES (max(1.-floor(mod(iTime/2./PI,3.)),0.)==1.? 6.: max(1.-floor(mod(iTime/2./PI-1.,3.)),0.)==1.? 1.:100.)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    {
        vec3 backgroundCol =    texture(iChannel0, uv).rgb     * max(1.-floor(mod(iTime/2./PI,3.)),0.)
                             +  texture(iChannel1, uv).rgb     * max(1.-floor(mod(iTime/2./PI-1.,3.)),0.)
                             +  texture(iChannel2,viewDir).rgb * max(1.-floor(mod(iTime/2./PI-2.,3.)),0.);
